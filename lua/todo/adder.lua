...
function Adder:add_highlight()
	...
	local ops = { "add", "delete", "clear", "done", "edit" }
	local hls = { "TodoAdd", "TodoDelete", "TodoClear", "TodoDone", "TodoEdit" }
	local pre_len = { 1, 2, 2, 2, 1 } -- to avoid ambiguity
...
function Adder:_create_main_window()
	...
	local _, win_id =
		utils.create_bufwin(self.opts.width, self.opts.adder_height, self.opts.row, self.opts.col, border, 1)
...
function Adder:_create_input_window()
	local buf, win_id = utils.create_bufwin(
		self.opts.width - #self.opts.prompt_prefix,
		self.opts.adder_height,
		self.opts.row + 1,
		self.opts.col + #self.opts.prompt_prefix,
		"none",
		2
	)
...
function Adder:_create_prefix_window()
	local buf, win_id =
		utils.create_bufwin(#self.opts.prompt_prefix, 1, self.opts.row + 1, self.opts.col + 1, "none", 2)
...
function Adder:_parse(line)
	...
	elseif #uop > 1 and string.match("delete", puop) then
		op = "delete"
		arg1 = tonumber(line:match("(%d+)%s*$")) or 1
	elseif string.match("clear", puop) then
		op = "clear"
		arg1 = 0 --unused
