...
function Previewer:new(opts)
	...
	self.opts = opts
	local border = { "├", "─", "┤", "│", "╯", "─", "╰", "│" }
	local buf, win_id = utils.create_bufwin(opts.width, opts.previewer_height, opts.row + 2, opts.col, border, 1)
...
function Previewer:_clear()
	for idx = self.todos + 1, #self.lines do
		self.lines[idx] = nil
	end
end
